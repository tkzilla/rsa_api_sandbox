...
def if_playback():
    ...
    ###########################################################
    # If a device is connected, PLAYBACK_OpenDiskFile() will crash
    print('\n########IF Playback########')
    cdef DEVICE_INFO devInfo
    rs = DEVICE_GetInfo(&devInfo)
    if rs != ReturnStatus.errorNotConnected:
        print('There is a device connected, disconnecting device.')
        DEVICE_Disconnect()
