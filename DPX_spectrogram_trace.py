...
def main():
	...
	print('Valid Traces in DPXogram: %d' % validTraces)
	print(fb.sogramBitmapWidth)
	sogram = np.empty((validTraces,dsStruct.bitmapWidth))
	...
	# Pack the data from DPX_FrameBuffer into a usable NumPy array
	...
	# The bitmap only contains usable data up to valid traces*bitmap width.
	# The bitmap structure itself is larger, but it contains mostly zero data.
	validDataIndex = int(fb.sogramBitmapWidth*validTraces)
	dpxogramBitmap = dpxogramBitmap[:validDataIndex].reshape(
		(validTraces,fb.sogramBitmapWidth))
	...
	plt.ylim(0, validTraces*timeResolution.value)
